from bat import BatAlgorithm
from bees import BeesAlgorithm
from firefly import FireflyAlgorithm
import numpy as np

rosenbrock_fct = lambda x: sum([100*(x[i+1]-x[i])**2+(1-x[i])**2 for i in range(len(x)-1)])
sphere_fct = lambda x: x[0]**2 + x[1]**2
rastrigin_fct = lambda x: 10*len(x)+sum([x[i]**2-10*np.cos(2*np.pi*x[i]) for i in range(len(x))])
himmelblau_fct = lambda x: (x[0]**2+x[1]-11)**2+(x[0]+x[1]**2-7)**2


objective_fct = rosenbrock_fct
d = 7


bat = BatAlgorithm(d=d, np=200, range_min=-5.0, range_max=5.0,
                   a=0.5, r=0.5, qmin=0.0, qmax=3.0)
solutions = bat.search(objective_fct, 100, k=1, visualize=False, minimize=True)
print(solutions)


bees = BeesAlgorithm(d=d, np=200, range_min=-5.0, range_max=5.0,
                     ne=40, nb=10, nre=10, nrb=3, shrink_factor=0.8, stgn_lim=3)
solutions = bees.search(objective_fct, 100, k=1, visualize=False, minimize=True)
print(solutions)

