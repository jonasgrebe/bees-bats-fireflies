...
class BatAlgorithm(SwarmSearchAlgorithm):
    ...
    def initialize(self):
        ...
        self.best = self.population[np.argmax([self.score_function(s) for s in self.population])]
    ...
    def single_search_step(self):
        for i in range(self.np):
            ...
            if (f_new >= self.score_function(self.population[i])) and (np.random.uniform(0, 1) < self.a):
    ...
    def highlight_population(self, ax):
        if self.d == 1:
            ax.scatter(self.best, self.score_function(self.bests))
        if self.d == 2:
